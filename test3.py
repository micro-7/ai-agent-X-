# twitter_agent_full.py
"""
Single-file Twitter agent (mock by default) — LangChain (OpenAI) + Tweepy + persistent DB + moderation + human-review CLI.
Features:
- MOCK_MODE (default true) runs local demo mention; no Twitter calls.
- Live mode: set MOCK_MODE=false and provide Twitter API keys in .env to fetch mentions and post replies.
- Persistent state and human-review queue via SQLite (SQLAlchemy).
- LLM via langchain_openai OpenAI class using modern .invoke(prompt).
- Retries for LLM calls (tenacity).
- Admin CLI to list/approve/reject/publish (simulate or real depending on config).
.env variables (example):
    ...
    TWITTER_USERNAME=your_twitter_username_or_empty
    MOCK_MODE=true
    PREVIEW_ONLY=true
    POLL_INTERVAL=20
...
Save this file and run:
    python twitter_agent_full.py        # run agent loop (mock by default)
    python twitter_agent_full.py --admin  # open admin CLI
"""
import os
import time
import sys
...
# LangChain OpenAI (modern split packages)
...
# LangChain OpenAI LLM instance (modern)
llm = OpenAI(openai_api_key=OPENAI_API_KEY, temperature=0.6, model_name="gpt-4o-mini")
...
def call_llm(prompt_text: str) -> str:
    """Invoke the LangChain OpenAI LLM and normalize result to a string."""
...
def fetch_mentions(client: tweepy.Client, user_id: str, since_id: Optional[str]) -> List[Dict]:
    mentions = []
    # Use get_users_mentions; signature varies across tweepy versions, handle common shapes
    try:
        if since_id:
...
def run_agent_once_mock(state: dict):
    ...
    if saved:
        ...
        # after generation, you can auto-approve for quick demo if desired:
        # approve(demo_mention["id"]); simulate_posting(demo_mention["id"])
    else:
        print("No new mentions.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--admin":
        ...
        print(f"Starting Twitter LangChain agent. MOCK_MODE={MOCK_MODE} PREVIEW_ONLY={PREVIEW_ONLY}")
        state = load_state()
        try:
            while True:
                try:
